...
/*   Updated: 2025/08/28 12:18:25 by ipogodae         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	find_binary(char *path_or_name, char **result_path, t_env *env)
{
	...
	if (ft_strchr(path_or_name, '/'))
	{
		...
		if (error_code)
			return (print_error("No such file or directory", 127));
		if (S_ISDIR(st.st_mode))
			return (print_error("Is a directory", 126));
		if (access(path_or_name, X_OK))
			return (print_error("Permission denied", 126));
...
)
{
	...
	else if (t->data[*i].type == TOKEN_REDIR_APPEND_ERR)
	{
		...
		return (process_in(&(c->heredoc), "No heredoc after <<", t, i));
	return (0);
}
